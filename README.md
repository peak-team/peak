...
| `PEAK_ALLOW_UNSAFE_GUM_PROLOGUE` | Allows PEAK to override its fail-closed attach guard for target prologues that Gum cannot safely relocate. On x86_64 this covers the audited `DL`/`EDX`/`RDX` zero, `EAX` zero, first indexed-load prefix family that Gum 16.5.9 corrupts or crashes in tests, early `mov imm` register prologues that crash in Frontera Intel tests, plus tiny functions whose decoded instruction stream returns inside Gum's first 32 relocated bytes; unrelated longer copy prologues stay attachable. On Arm64 this conservatively covers `x16`/`x17` (`ip0`/`ip1`) live-out prologues. Set only for diagnostics when you accept that risk. |
