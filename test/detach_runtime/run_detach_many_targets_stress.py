...
def format_cycle_examples(cycles, limit=3):
    ...
    for cycle in cycles[:limit]:
        parts.append(
            ...
            "d2={second_detach_time:.6f};call_delta={call_delta}"
...
def parse_trace(path, hot_targets):
    result = {
        "rows": 0,
        "detach_success": 0,
        "reattach_success": 0,
    ...
    for hook_id, events_for_hook in hook_events.items():
        ...
        for event in sorted(events_for_hook, key=lambda item: item["time"]):
            ...
            if ordered_reattach is not None and call_delta > 0:
                result["strict_reattach_ordered_cycles"] += 1
                result["strict_reattach_ordered_call_delta_total"] += call_delta
                result["strict_reattach_ordered_call_delta_max"] = max(
                    result["strict_reattach_ordered_call_delta_max"],
                    call_delta,
                )
                ordered_distinct_hooks.add(hook_id)
                if len(ordered_examples) < 3:
    ...
    result["strict_reattach_ordered_distinct_hooks"] = len(ordered_distinct_hooks)
    result["strict_reattach_ordered_cycle_examples"] = ordered_examples
...
def compute_strict_hot_reattach_cycles(trace, samples_by_index, hot_targets):
    result = {
        "cycles": 0,
        "distinct_hooks": 0,
        "global_cycles": 0,
...
def run_once(args):
    ...
    try:
        ...
        trace = parse_trace(trace_path, args.hot_targets)
        print(
