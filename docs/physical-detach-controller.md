...
On Linux, the default helper launch path uses `fork()`, matching the
long-tested behavior.
Set `PEAK_DETACH_HELPER_SPAWN=clone`, `clone-vfork`, or `no-atfork` to opt into a raw
`clone(CLONE_VM | CLONE_VFORK | SIGCHLD)` launch path that immediately
`execve()`s the helper while avoiding libc `fork()`/`pthread_atfork()`
handlers. Explicit `PEAK_DETACH_BACKEND=helper` may warm this helper before the
first mutation. Strict `auto` does not eagerly start the helper during PEAK
initialization; it starts the helper only when the first real detach/reattach
mutation needs a backend, then falls back to the signal backend only for
structured helper unavailability, permission, timeout, or unsupported outcomes.
This preserves helper-first `auto` semantics while avoiding a rank-wide helper
startup storm before user work begins.
